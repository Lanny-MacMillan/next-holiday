...
export const fetchNewYearTasks = () => async (dispatch: any) => {
	...
	try {
		...
		dispatch(setNewYearTasksError(error instanceof Error ? error.message : "Unknown error"));
	}
};

// Async thunk for creating a new New Year task
export const createNewYearTask = (taskData: Omit<Task, "id" | "createdAt" | "updatedAt">) => async (dispatch: any) => {
	try {
		const newTask: Task = {
			...taskData,
