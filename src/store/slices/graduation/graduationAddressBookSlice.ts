...
export const addGraduationContact = createAsyncThunk(
	...
	async (
		contact: Omit<GraduationContact, "id" | "createdAt" | "updatedAt">
	) => {
		// Simulate API call
...
const graduationAddressBookSlice = createSlice({
	...
	reducers: {
		setSelectedContact: (
			state,
			action: PayloadAction<GraduationContact | null>
		) => {
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(fetchGraduationContacts.rejected, (state, action) => {
				state.loading = false;
				state.error =
					action.error.message || "Failed to fetch graduation contacts";
			})
			...
			.addCase(addGraduationContact.rejected, (state, action) => {
				state.loading = false;
				state.error =
					action.error.message || "Failed to add graduation contact";
			})
			...
			.addCase(updateGraduationContact.rejected, (state, action) => {
				state.loading = false;
				state.error =
					action.error.message || "Failed to update graduation contact";
			})
			...
			.addCase(deleteGraduationContact.rejected, (state, action) => {
				state.loading = false;
				state.error =
					action.error.message || "Failed to delete graduation contact";
			});
	},
});

export const { setSelectedContact, clearError } =
	graduationAddressBookSlice.actions;
