...
const birthdayAddressBookSlice = createSlice({
  ...
  initialState,
  reducers: {
    setSelectedContact: (state, action: PayloadAction<BirthdayContact | null>) => {
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(fetchBirthdayContacts.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to fetch birthday contacts';
      })
      ...
      .addCase(updateBirthdayContact.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to update birthday contact';
      })
      ...
      .addCase(deleteBirthdayContact.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to delete birthday contact';
      });
  },
});

export const { setSelectedContact, clearError } = birthdayAddressBookSlice.actions;
