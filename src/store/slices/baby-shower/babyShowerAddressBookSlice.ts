...
export const addBabyShowerContact = createAsyncThunk(
  ...
  async (contact: Omit<BabyShowerContact, 'id' | 'createdAt' | 'updatedAt'>) => {
    // Simulate API call
...
const babyShowerAddressBookSlice = createSlice({
  ...
  initialState,
  reducers: {
    setSelectedContact: (state, action: PayloadAction<BabyShowerContact | null>) => {
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(fetchBabyShowerContacts.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to fetch baby shower contacts';
      })
      ...
      .addCase(addBabyShowerContact.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to add baby shower contact';
      })
      ...
      .addCase(updateBabyShowerContact.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to update baby shower contact';
      })
      ...
      .addCase(deleteBabyShowerContact.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to delete baby shower contact';
      });
  },
});

export const { setSelectedContact, clearError } = babyShowerAddressBookSlice.actions;
