...
export default function BirthdayPartyPlanningPage() {
	...
	const partyPlanning = holidayData?.tasks?.filter((task: any) => task.category === "Party Planning") || [];
	...
	const incompletePartyPlanningTasks = sortedTasks.filter((task: any) => !task.isCompleted);
	const completedPartyPlanningTasks = sortedTasks.filter((task: any) => task.isCompleted);
			...
			<main className="flex-1 w-full max-w-4xl flex flex-col gap-6 mt-4">
				...
				{showDefaultTasks && (
					<div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
						<h3 className="text-lg font-semibold mb-4">Get Started with Party Planning</h3>
				...
				{partyPlanning.length > 0 && (
					<div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
						...
						<div className="grid grid-cols-3 gap-4 text-center">
							<div>
								<div className="text-2xl font-bold text-blue-600">{partyPlanning.length}</div>
								<div className="text-sm text-gray-600 dark:text-gray-400">Total Tasks</div>
							</div>
							<div>
								<div className="text-2xl font-bold text-green-600">{completedPartyPlanningTasks.length}</div>
								<div className="text-sm text-gray-600 dark:text-gray-400">Completed</div>
							</div>
							<div>
								<div className="text-2xl font-bold text-orange-600">{incompletePartyPlanningTasks.length}</div>
