...
import {
	fetchMothersDayTasks,
	addMothersDayTask,
	updateMothersDayTask,
	deleteMothersDayTask,
	toggleMothersDayTaskCompletion,
	MothersDayTask,
} from "@/store/slices/mothers-day/mothersDayTasksSlice";
...
export default function MothersDayEventsPage() {
	...
	const tasks = useAppSelector((state) => state.mothersDayTasks.tasks);
	const error = useAppSelector((state) => state.mothersDayTasks.error);
	const loading = useAppSelector((state) => state.mothersDayTasks.loading);
	...
	const handleSubmit = async (values: Record<string, any>) => {
		if (editingTask) {
			await dispatch(updateMothersDayTask({ ...editingTask, ...values }));
			setEditingTask(null);
		} else {
			await dispatch(
				addMothersDayTask({
	...
	const confirmDelete = async () => {
		if (taskToDelete) {
			await dispatch(deleteMothersDayTask(taskToDelete.id));
	...
	const handleToggleCompletion = async (taskId: string) => {
		await dispatch(toggleMothersDayTaskCompletion(taskId));
	};
