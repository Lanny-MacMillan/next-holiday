...
export default function EasterBasketListPage() {
	...
	const handleDelete = async (giftId: string) => {
		if (confirm("Are you sure you want to delete this basket item?")) {
				...
				{gifts.length > 0 && (
					<div className="card rounded-lg p-4">
						...
						<div className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
							<div>Total Items: {gifts.length}</div>
					...
					<div className="card card-gifts rounded-lg p-6 max-w-md mx-4 w-full max-h-[90vh] overflow-y-auto">
						<div className="flex justify-between items-center mb-4">
							<h3
								...
								style={{ color: "#111827" }}
							>
								{editingGift ? "Edit Basket Item" : "Add New Basket Item"}
							</h3>
							<button
								onClick={() => {
									setShowAddForm(false);
						...
						<form onSubmit={handleSubmit} className="space-y-4">
							...
							<div className="flex gap-2">
								<button
									...
									style={{ backgroundColor: "#a855f7", color: "white" }}
								>
									{editingGift ? "Update Item" : "Add Item"}
								</button>
								<button
									type="button"
									onClick={() => {
