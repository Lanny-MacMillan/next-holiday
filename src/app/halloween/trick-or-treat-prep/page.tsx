...
export default function HalloweenTrickOrTreatPrepPage() {
	...
	function handleAddTask(e: React.FormEvent) {
		e.preventDefault();
		if (form.title.trim()) {
			dispatch(addHalloweenTask({ ...form, isCompleted: false }));
			setForm({
				title: "",
				description: "",
				priority: "medium" as "low" | "medium" | "high",
		...
		<div className="min-h-screen halloween-gradient flex flex-col items-center p-4 sm:p-8 font-sans">
			<header className="w-full max-w-2xl py-6 flex flex-col items-center relative">
				...
				<h1 className="text-3xl font-bold mb-2 text-gray-800 dark:text-white">
					🕯️ Trick-or-Treat Prep
				</h1>
				<p className="text-center text-gray-600 dark:text-gray-400">
					Get ready for a spooky night of trick-or-treating!
				</p>
			</header>
				...
				<div className="space-y-3">
					...
					{sortedTasks.length === 0 && (
						<div className="text-center py-8 text-gray-500 dark:text-gray-400">
							No trick-or-treat prep tasks yet. Add your first task!
						</div>
					)}
				</div>
			</main>
