...
import {
	fetchFathersDayGifts,
	addFathersDayGift,
	updateFathersDayGift,
	deleteFathersDayGift,
	toggleFathersDayGiftCompletion,
	FathersDayGift,
} from "@/store/slices/fathers-day/fathersDayGiftListSlice";
...
export default function FathersDayGiftListPage() {
	...
	const { gifts, loading, error, initialized } = useAppSelector(
		(state: any) => state.fathersDayGiftList
	);
	...
	const [editingGift, setEditingGift] = useState<FathersDayGift | null>(null);
	...
	function handleAddGift(values: Record<string, any>) {
		...
		if (editingGift) {
			...
			dispatch(updateFathersDayGift(updatedGift));
			setEditingGift(null);
		} else {
			// Add new gift
			const newGift: Omit<FathersDayGift, "id" | "createdAt" | "updatedAt"> = {
				name: values.name,
				description: values.description || undefined,
				price: parseFloat(values.price) || 0,
				recipient: values.recipient,
			...
			dispatch(addFathersDayGift(newGift));
		}
	...
	function handleToggleGift(giftId: string) {
		dispatch(toggleFathersDayGiftCompletion(giftId));
	}
	...
	function handleEditGift(gift: FathersDayGift) {
		setEditingGift(gift);
		setShowForm(true);
	}
	...
	function confirmDelete() {
		if (deleteConfirm.giftId) {
			dispatch(deleteFathersDayGift(deleteConfirm.giftId));
	...
	function sortGifts(giftsToSort: FathersDayGift[]): FathersDayGift[] {
		switch (sortBy) {
			case "recipient":
	...
	const incompleteGifts = sortedGifts.filter(
		(gift: FathersDayGift) => !gift.isCompleted
	);
	const completedGifts = sortedGifts.filter(
		(gift: FathersDayGift) => gift.isCompleted
	);

	const renderGiftItem = (gift: FathersDayGift) => (
		<GiftCardItem
			key={gift.id}
			gift={gift}
			isCompleted={false}
	...
	const renderCompletedGiftItem = (gift: FathersDayGift) => (
		<GiftCardItem
			key={gift.id}
			gift={gift}
			isCompleted={true}
