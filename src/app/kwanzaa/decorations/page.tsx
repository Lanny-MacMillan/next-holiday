...
export default function KwanzaaDecorationsPage() {
	...
	console.log('Kwanzaa Decorations Debug:', {
		resolvedHolidayId,
			...
			for (const task of defaultDecorationTasks) {
				...
				try {
					...
					if (response.ok) {
						...
						dispatch(removeTaskFromHomeData({ holidayId: resolvedHolidayId, taskId: newTask.id }));
						console.error("Failed to add default decoration:", response.status, response.statusText);
					}
				} catch (taskError) {
					dispatch(removeTaskFromHomeData({ holidayId: resolvedHolidayId, taskId: newTask.id }));
	...
	async function handleEditDecoration(values: Record<string, any>) {
		if (!values.title?.trim() || !editingTask || !resolvedHolidayId || !auth0User) return;
	...
	const incompleteDecorations = sortedDecorations.filter((decoration: any) => !decoration.isCompleted);
	const completedDecorations = sortedDecorations.filter((decoration: any) => decoration.isCompleted);
