...
export default function ValentinesGiftListPage() {
	...
	function confirmDelete() {
		if (deleteConfirm.giftId) {
			dispatch(deleteValentinesGift(deleteConfirm.giftId));
		}
	...
	function addFromAddressBook(contact: any) {
		setForm((prev) => ({ ...prev, recipient: contact.name }));
		setShowAddressBook(false);
	}

	function sortGifts(giftsToSort: ValentinesGift[]): ValentinesGift[] {
		switch (sortBy) {
			case "recipient":
		...
		<div className="min-h-screen valentines-gradient flex flex-col items-center p-4 sm:p-8 font-sans">
			<header className="w-full max-w-md py-6">
				<div className="flex items-center justify-center relative">
					...
					<button
						...
						className="absolute right-0 text-pink-600 hover:text-pink-800 dark:text-pink-400 dark:hover:text-pink-300 text-xl"
						title="Sort gifts"
					>
			...
			<main className="w-full max-w-md flex flex-col gap-6">
				...
				<div>
					...
					<div className="card card-valentines rounded shadow">
						{incompleteGifts.length === 0 ? (
							...
							<ul className="divide-y divide-gray-200 dark:divide-gray-700">
								{incompleteGifts.map((gift: ValentinesGift) => (
									<li
										key={gift.id}
										className="flex items-center px-4 py-3 cursor-pointer hover:bg-pink-50 dark:hover:bg-pink-900/20"
				...
				<div>
					...
					<div className="card card-valentines rounded shadow">
						{completedGifts.length === 0 ? (
							...
							<ul className="divide-y divide-gray-200 dark:divide-gray-700">
								{completedGifts.map((gift: ValentinesGift) => (
									<li
										key={gift.id}
										className="flex items-center px-4 py-3 cursor-pointer hover:bg-pink-50 dark:hover:bg-pink-900/20 opacity-60"
										onClick={() => handleToggleGift(gift.id)}
									>
										<input
											type="checkbox"
			...
			{deleteConfirm.show && (
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
					<div className="card card-valentines rounded-lg p-6 max-w-sm mx-4">
