...
export default function ValentinesDateIdeasPage() {
	...
	const { contacts, isHolidayShared } = useAppSelector((state: any) => state.addressBook);
	...
	console.log('Valentine Date Ideas Debug:', {
		...
		dateIdeas: dateIdeas.map(d => ({ id: d.id, title: d.title, category: d.category, isCompleted: d.isCompleted }))
	});
		...
		try {
			...
			if (response.ok) {
				...
				console.log('🐛 [RefreshHomeData] Response received:', {
					...
					holidayData: result.data?.holidays?.find((h: any) => h.id === resolvedHolidayId)
				});
				dispatch(setHomeData(result.data));
				console.log('🐛 [RefreshHomeData] Redux state updated');
			} else {
				console.error('🐛 [RefreshHomeData] Response not ok:', response.status, response.statusText);
			}
		} catch (error) {
	...
	const handleUpdateDateIdea = async (values: Record<string, any>) => {
		...
		try {
			...
			if (response.ok) {
				...
				console.log('🐛 [ValentineDateIdeasUpdate] Current dateIdeas after Redux update:', 
					dateIdeas.map(d => ({ id: d.id, title: d.title, dueDate: d.dueDate, updatedAt: d.updatedAt }))
				);
	...
	const handleToggleCompletion = async (taskId: string) => {
		...
		try {
			...
			if (task) {
				console.log('🐛 [ToggleCompletion] Before:', { taskId, currentCompleted: task.isCompleted });
	...
	const handleDeleteConfirm = async () => {
		...
		try {
			const response = await fetch(`/api/holidays/${resolvedHolidayId}/tasks/${taskToDelete.id}`, {
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
					"x-test-user": JSON.stringify(auth0User),
			...
			if (response.ok) {
				dispatch(removeTaskFromHomeData({ 
					holidayId: resolvedHolidayId, 
