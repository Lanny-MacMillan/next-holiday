...
export default function CountdownTimer({
  ...
  const dispatchUpdateCountdown = (holiday: string | undefined, date: string) => {
    switch (holiday) {
      case 'Hanukkah':
  ...
  if (!timeLeft) {
    return (
      <div
        className={`${className} text-xs relative z-20 min-h-[20px] flex items-center justify-end ${
          isGamifiedMode
    ...
    return (
      ...
      >
        <button
          ...
          className={`text-xs cursor-pointer transition-all duration-200 font-medium whitespace-nowrap ${
            isGamifiedMode
