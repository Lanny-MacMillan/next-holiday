...
const DecorationsListItem: React.FC<DecorationsListItemProps> = ({
  ...
  const isGradientClass = holidayColor && holidayColor.startsWith('bg-gradient');
  ...
  const DecorationIcon = ({ priority }: { priority: string }) => {
    ...
    return <div className="text-xl sm:text-2xl">{getIcon(priority)}</div>;
  };
  ...
  if (task.isCompleted) {
    if (isGamifiedMode) {
      ...
      return (
        <li
          key={task.id}
          className={`relative card rounded-2xl p-3 sm:p-4 cursor-pointer transition-all duration-200 hover:scale-[1.02] active:scale-100 overflow-hidden opacity-60 text-white tracking-wide border-2 border-white border-opacity-30 ${
            isGradientClass ? holidayColor : ''
          }`}
          style={{
            backgroundColor: isGradientClass ? undefined : holidayColor || '#6b7280', // gray-500 fallback
            ...getCardStyling({
              isDarkMode,
              isGamified: true,
              intensity: 'heavy',
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-start space-x-3">
              {/* Decoration Icon */}
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white bg-opacity-10 rounded-full flex items-center justify-center backdrop-blur-sm flex-shrink-0">
              ...
              >
                <div className="font-semibold text-white line-through opacity-60 text-sm sm:text-base">
                  {task.title}
                </div>
                {task.description && (
    ...
    return (
      <li
        key={task.id}
        className="flex items-center px-3 py-3 sm:px-4 sm:py-3 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 opacity-60"
        onClick={handleToggleTask}
      >
        <input
          type="checkbox"
  ...
  if (isGamifiedMode) {
    ...
    return (
      <li
        key={task.id}
        className={`relative card rounded-2xl p-3 sm:p-4 cursor-pointer transition-all duration-200 hover:scale-[1.02] active:scale-100 overflow-hidden text-white tracking-wide border-2 border-white border-opacity-30 ${
          isGradientClass ? holidayColor : ''
        }`}
        style={{
          backgroundColor: isGradientClass ? undefined : holidayColor || '#3b82f6', // blue-500 fallback
          ...getCardStyling({
            isDarkMode,
            isGamified: true,
            intensity: 'heavy',
  ...
  return (
    <li
      key={task.id}
      className="flex items-center px-3 py-3 sm:px-4 sm:py-3 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20"
      onClick={handleToggleTask}
    >
      <input
        type="checkbox"
