...
const SKIP_REPLACEMENT = '{ skip: shouldSkipHolidayQuery(holidayId, auth0User, currentState) }';
...
function updateHolidayPage(filePath) {
  ...
  try {
    ...
    if (!content.includes('shouldSkipHolidayQuery')) {
      ...
      content = content.slice(0, nextLineIndex) + IMPORT_ADDITION + '\n' + content.slice(nextLineIndex);
    }
    
    // Replace hooks section
    const hooksPattern = /const holidayPreferences = useAppSelector\([\s\S]*?const holidayId = getHolidayIdFromRoute\([^)]+\);/;
