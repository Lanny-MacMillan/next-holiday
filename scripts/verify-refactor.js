...
HOLIDAY_PAGES.forEach(filePath => {
  try {
    ...
    const hasOldPattern = content.includes('{ skip: !holidayId || !auth0User }');
...
const UTILITY_FILES = ['src/utils/holidayData.ts', 'src/store/selectors/home.ts'];
...
try {
  ...
  if (packageJson.includes('guard:fetch') && packageJson.includes('guard:storage')) {
